...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['product_id'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT idpanier, quantity FROM panier WHERE idproduit = :product_id AND iduser = :user_id");
        ...
        if ($cart_item) {
            ...
            if ($product) {
                $price = $product['price'];
                $insert_stmt = $conn->prepare("INSERT INTO panier (idproduit, iduser, quantity, prixtotal) VALUES (:product_id, :user_id, 1, :price)");
